...
-   Traffic congestion is a global issue, and public transportation is one of the main ways cities are trying to improve and reduce congestion. Car Sharing Services is relatively new option of public transportation, offers an alternative to private cars. One of the intriguing questions, is about the willingness of the public to give up the private car, and the connection to the socio-economic Status.

-   Our research hypothesis is that wealthier people will prefer to own their privete cars, while the other classes will prefer car-sharing services to save car maintenance costs.

-   We chose the AutoTel program, operated by the city of Tel Aviv, as an example of car sharing service. Our goal is to analyze the relation between socio-economic status of Tel Aviv residents and the usage patterns of the AutoTel car-sharing.

-   In order to examine the hypothesis, We collected data on uses of AutoTel sharing cars, that includes the geographic coordinates of the cars during the whole year of 2022. Then We aggregated the data to statistical zones by performing a spatial join.  The second set of data we collected, includes socio-economic status indicators by statistical zones. After collecting the data, We analyzed the connection between specific socioeconomic variables, and the use of AutoTel car-sharing service at statistical zones

## Literature 1:

#### Car-Sharing Services: An Annotated Review

-   Car sharing services offer flexibility of private mobility, without the need for individual vehicle ownership, thus actually transforming private mobility into a public service. This new form of transportation, is increasing the mobility of public transportation users.

-   Adding this service, can raise awareness of the social and environmental impacts of private car usage and supports multi-modal transportation. Car sharing services allows people to combine different ways of travel such as trains, buses, and private cars in a single trip.

-   there are three kinds of sharing car services:

1.  **One-way** - The cars are parked at parking lots that are defined by the service provider, and the trips must start and end at the same place.

2.  **Free-floating** - The parking lot where the trip ends can be different from the parking lot where it started, in a variety of parking lots predefined by the operator.
...
-   Experience shows that car-sharing services are more popular in high-density neighborhoods, where public transportation is more efficient and private car use is less frequent. It suggests that sharing car can be supplementary component of the public transportation services
...
-   The study hypothesis is that human patterns are related to their regional sociology-economic characteristics, such as income per capita and poverty rate.

-   The study uses two types of socioeconomic data - housing prices in Singapore and per capita income in Boston.

-   The level of mobility is estimated through the spatial scope of a person’s activity through space: how regularly his mobility is on daily basis, the variety of movements between important activity locations, and the unity of mobility traces.

-   Findings shows different patterns at each city: Wealthier user groups tend to travel [shorter]{.underline} distances in Singapore relative to other groups, while the opposite in Boston - Wealthier user groups tend to travel [farther]{.underline} then other groups. (Other mobility indicators, such as the number of activity locations, activity and travel variety, show similar characteristics in different socioeconomic classes).

-   The conflicting findings of the article on both cities shows a complected relation between travel distance and wealth, and suggest that results can be largely explained by spatial shape of cities:  In the United States, wealthier people live in the suburbs and more disadvantaged people occupy city centers, and the opposite in Singapore.

-   We can conclude then, that patterns of mobility are not always global, and influenced from the spatial arrangement of cities themselves. We can also conclude that there is a global (and maybe expected) pattern: wealth influences mobility, to the benefit of the higher socioeconomic groups, adopted to the local conditions.
...
-   The sharing economy is a way to reduce the environmental impact and costs of products and increase the availability of means of transportation ,Car sharing is a part of the sharing economy that gets people use cars when needed without having to buy one.

-   study surrounding the motivation of people to use shared vehicles, found that economic benefits are the most influential in a user’s choice to use a car sharing service: Hence car sharing can be considered as an alternative viable mobility solution, and that the economic benefits are the strongest motivating factors for using car sharing services.

## Literature 4:

#### Car sharing adoption intention in urban areas: What are the key Socioeconomic drivers?

-   car sharing service is common in high-density metropolitan areas, which allows and avoids congestion, parking and pollution problems. Choosing a sharing car means reducing the length of trips and reducing emissions.

-   An encouraging way for the use of shared car service are restrictions on parking in dense areas of the city which strengthens the potential for the use of shared cars.
...
-   Other indications shows that  drivers who drive infrequently, tend to use  more car-sharing services, suggesting that primary drivers have a greater need for continuous access to their private car.

## Indices For Testing The Hypothesis:

The articles provide a scientific background for the research,and suggests that using car-sharing services tend to be impacted by the economic status of users.
...
-   **Cars_per_capita**: Using in sharing cars divided by population
...
lamas data tables on statistical zones
...
Using the corrgram function, we cobserve there is a weak to strong correlation between the variables, that is showing the linearity of their releations.ther is Multicollinearity for the variables: yeesAnnual_med, Parking_pcnt, AcadmCert_pcnt, and employedAnnual_me. This means these variables are **highly correlated** with each other and not only with a single variable. In this cases, the model remains useful for prediction but not suitable for measuring the individual influence of these variables due to independence of observations.
...
### After Linear Transformation, Severe Left-Skewed Data Is Still Not Normal
...
We can conclude that there is no a significant relation between car sharing services usage and socioeconomic status.

after statistical analyse using linear regression model None of the indicators predict high usage of car sharing service , It means there is no connection between them.

Future studies can continue investigating with more data to find the reason of the relation, between the field that we examined of economics and transportation.
